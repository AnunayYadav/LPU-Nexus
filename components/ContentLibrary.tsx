...
const SkeletonCard = () => (
  <div className="group p-5 rounded-[30px] border border-slate-100 dark:border-white/5 bg-white dark:bg-dark-900 shadow-sm relative overflow-hidden flex flex-col min-h-[140px] animate-pulse">
    <div className="w-10 h-10 bg-slate-200 dark:bg-dark-800 rounded-xl mb-4 shimmer" />
    <div className="h-4 w-3/4 bg-slate-200 dark:bg-dark-800 rounded-md mb-2 shimmer" />
    <div className="h-3 w-1/2 bg-slate-200 dark:bg-dark-800 rounded-md shimmer" />
  </div>
);

// Fix: Added missing interface for ContentLibrary component props
interface ContentLibraryProps {
  userProfile: UserProfile | null;
  initialView?: 'browse' | 'my-uploads';
}

// ... (fetchFromSource and logic remain identical)
...
const ContentLibrary: React.FC<ContentLibraryProps> = ({ userProfile, initialView = 'browse' }) => {
  ...
  const displayFiles = useMemo(() => {
    let data = [...allFiles];
    if (isAdminView || viewMode === 'my-uploads' || searchQuery.trim() !== '') { /* flattened */ } 
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in pb-20 px-4 md:px-0">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tighter uppercase mb-1">Library Hub</h2>
          <nav className="mt-2 flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
            <button onClick={() => navigateTo(null, null, null)} className="hover:text-orange-500 transition-colors border-none bg-transparent uppercase">Root</button>
            {activeSemester && <><span className="opacity-30">/</span><button onClick={() => navigateTo(activeSemester, null, null)} className="hover:text-orange-500 border-none bg-transparent">{activeSemester.name}</button></>}
            {activeSubject && <><span className="opacity-30">/</span><button onClick={() => navigateTo(activeSemester, activeSubject, null)} className="hover:text-orange-500 border-none bg-transparent">{activeSubject.name}</button></>}
          </nav>
        </div>
        <div className="flex gap-2">
           {userProfile?.is_admin && (
             <>
                <button onClick={() => { setIsAdminView(!isAdminView); setViewMode('browse'); navigateTo(null, null, null); }} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all border-none ${isAdminView ? 'bg-orange-600 text-white' : 'bg-dark-900 text-slate-400'}`}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-5 h-5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></button>
                <button onClick={() => setShowFolderModal(true)} className="w-10 h-10 bg-dark-900 rounded-xl flex items-center justify-center text-orange-600 transition-all border-none shadow-sm hover:bg-dark-800"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-5 h-5"><path d="M12 5v14M5 12h14"/></svg></button>
             </>
           )}
           <button onClick={() => { setViewMode(viewMode === 'browse' ? 'my-uploads' : 'browse'); navigateTo(null, null, null); }} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all border-none ${viewMode === 'my-uploads' ? 'bg-orange-600 text-white shadow-lg' : 'bg-dark-900 text-slate-400'}`}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-5 h-5"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></button>
           <button onClick={() => fileInputRef.current?.click()} className="px-5 py-2 bg-orange-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-orange-600/20 border-none active:scale-95 flex items-center gap-2">Contribute</button>
        </div>
      </header>
      ...
      <div className="relative flex-1">
        ...
        <input type="text" placeholder="Filter registry..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-11 pr-4 py-3.5 bg-white dark:bg-dark-900 border border-slate-200 dark:border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-orange-500 transition-all dark:text-white" />
      </div>
          ...
          <>
            {currentFolders.map(folder => (
              <div key={folder.id} onClick={() => { if (folder.type === 'semester') navigateTo(folder, null, null); else if (folder.type === 'subject') navigateTo(activeSemester, folder, null); else if (folder.type === 'category') navigateTo(activeSemester, activeSubject, folder); }} className="group p-6 rounded-[30px] border border-slate-100 dark:border-white/5 bg-white dark:bg-dark-900 hover:border-orange-500/50 hover:shadow-xl transition-all cursor-pointer relative overflow-hidden flex flex-col justify-center min-h-[140px]">
                ...
                <div className="absolute -right-2 -bottom-2 opacity-[0.03] group-hover:scale-110 transition-transform"><FolderIcon type={folder.type} size="w-24 h-24" /></div>
              </div>
            ))}
            {displayFiles.map(file => (
              <FileCard key={file.id} file={file} userProfile={userProfile} isAdminMode={isAdminView} onAccess={() => NexusServer.getFileUrl(file.storage_path).then(url => window.open(url, '_blank'))} onShowDetails={() => { setSelectedFile(file); setShowDetailsModal(true); }} />
            ))}
          </>
        )}
      </div>
      ...
      {showDetailsModal && selectedFile && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl animate-fade-in">
          <div ref={modalRef} className="bg-dark-950 rounded-[48px] w-full max-w-2xl border dark:border-white/10 shadow-[0_32px_128px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[90vh]">
            <header className="p-8 md:p-12 border-b dark:border-white/5 bg-dark-900 flex items-start justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-10 no-scrollbar">
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[{ label: 'Sem', val: selectedFile.semester }, { label: 'Subject', val: selectedFile.subject }, { label: 'Type', val: selectedFile.type }, { label: 'Size', val: selectedFile.size }].map((item, i) => (
                    <div key={i} className="p-4 bg-white/[0.03] rounded-2xl border dark:border-white/5">
               ...
               <div className="space-y-4">
                  ...
                  <p className="text-sm font-medium text-slate-300 leading-relaxed italic bg-white/[0.02] p-6 rounded-3xl border dark:border-white/5">{selectedFile.description || "Synthesized analysis complete."}</p>
               </div>
            </div>
            <footer className="p-8 md:p-12 bg-dark-900 border-t dark:border-white/5 flex gap-4">
               ...
               {selectedFile.status === 'approved' && (
                 <button onClick={() => { NexusServer.getFileUrl(selectedFile.storage_path).then(url => window.open(url, '_blank')); }} className="flex-[2] py-4 bg-orange-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all border-none font-black">Access Protocol ↗</button>
               )}
            </footer>
          </div>
        </div>
...
const FileCard: React.FC<{ file: LibraryFile; userProfile: UserProfile | null; isAdminMode: boolean; onAccess: () => void; onShowDetails: () => void; }> = ({ file, userProfile, isAdminMode, onAccess, onShowDetails }) => {
  return (
    <div onClick={onShowDetails} className="group p-5 rounded-[30px] border border-slate-100 dark:border-white/5 bg-white dark:bg-dark-900 hover:border-orange-500/50 hover:shadow-xl transition-all relative overflow-hidden flex flex-col min-h-[160px] cursor-pointer">
      <div className="flex items-start justify-between mb-2">
        <div className="w-9 h-9 bg-dark-950 rounded-xl flex items-center justify-center group-hover:text-orange-500 transition-colors"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-5 h-5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg></div>
      ...
      <div className="pt-3 mt-auto border-t dark:border-white/5 flex items-center justify-between">
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{file.size}</span>
        <button onClick={(e) => { e.stopPropagation(); onAccess(); }} className="bg-dark-950 text-orange-600 px-4 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-1.5 hover:bg-orange-600 hover:text-white transition-all shadow-md border-none">Access</button>
      </div>
    </div>
  );
};
