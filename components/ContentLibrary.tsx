...
const CustomDropdown: React.FC<{
  ...
  const updateCoords = () => {
    if (dropdownRef.current) {
      ...
      // Since the portal is fixed, use viewport coordinates (getBoundingClientRect)
      // without adding window.scrollY/scrollX.
      setCoords({
        top: rect.bottom + 8, // Add a small gap (replaces mt-2)
        left: rect.left,
        width: rect.width
      });
    }
  ...
  useLayoutEffect(() => {
    if (isOpen) {
      updateCoords();
      // Use capture phase for scroll to catch events from scrollable parents (like the modal)
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {/* 
        Fix: Check coords.width > 0 to prevent the dropdown from 
        rendering at (0,0) for one frame, which causes the "flying" glitch.
      */}
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-20 px-4 md:px-0">
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
           <div className="bg-white dark:bg-slate-950 rounded-[40px] w-full max-w-md shadow-2xl border border-white/10 relative flex flex-col max-h-[90vh] overflow-hidden">
              {!uploadSuccess ? (
                <>
