...
const SocialHub: React.FC<{ userProfile: UserProfile | null }> = ({ userProfile }) => {
  ...
  useEffect(() => {
    // Immediate scroll when view/conversation changes or loading finishes
    if (!isLoading) {
      scrollToBottom('auto');
    }
      ...
      {activeView !== 'directory' && activeView !== 'lounge' && activeView !== 'requests' && (
        <div className="w-64 md:w-80 bg-slate-50 dark:bg-[#080808] border-r border-slate-200 dark:border-white/5 flex flex-col hidden md:flex">
          <div className="p-6 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-white">{activeView === 'dms' ? 'Messages' : 'Groups'}</h3>
            <button onClick={loadConversations} className="p-1.5 hover:text-orange-500 transition-colors" title="Refresh"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-4 h-4"><path d="M23 4v6h-6"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button>
          </div>
                  ...
                  <div key={profile.id} className="glass-panel p-6 rounded-[32px] border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#050505] hover:border-orange-500/50 transition-all group flex flex-col relative overflow-hidden">
                    ...
                    <div className="min-h-[48px] mb-6">
                      {profile.is_public ? (
                        ...
                        <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black opacity-30">Profile Details Hidden</p>
                      )}
                    </div>
                    ...
                    <div className="mt-auto flex gap-2">
                      ...
                      {isPending && (
                        <span className="px-4 py-3 bg-slate-100 dark:bg-white/5 text-slate-400 rounded-xl font-black text-[9px] uppercase tracking-widest">Pending</span>
                      )}
                    </div>
                  </div>
                );
            ...
            <div className="space-y-12">
              <section>
                ...
                {inboundRequests.length === 0 ? (
                  <p className="text-slate-500 text-xs font-bold uppercase tracking-widest opacity-40">No pending requests.</p>
                  ...
                  <div key={req.id} className="glass-panel p-6 rounded-[32px] bg-white dark:bg-[#050505] border border-slate-200 dark:border-white/5 flex items-center justify-between mb-4">
                     <div className="flex items-center gap-4">
                       ...
                       <div>
                         ...
                         <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Awaiting your approval</p>
                       </div>
                     </div>
